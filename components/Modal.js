...
const Modal = ({ handleClose, type }) => {
    ...
    return (
        <Backdrop onClick={handleClose}>
            {type === "dropIn" && <motion.div
                onClick={(e) => e.stopPropagation()}
                className={`rounded-xl flex flex-col justify-center ${darkMode ? 'bg-[#1D2226]' : 'bg-white'} w-full max-w-lg md:-mt-96 mx-6`}
                variants={dropIn}
                initial="hidden"
                animate="visible"
                exit="exit"
            >
                <div className='flex items-center justify-between border-b border-white/75 px-4 py-2.5'>
                    ...
                    <IconButton onClick={handleClose}>
                        <CloseRoundedIcon className={`h-7 w-7 ${darkMode && 'text-white/75'}`} />
                    </IconButton>
                </div>
            ...
            {type === "gifYouUp" && (
                ...
                >
                    ...
                    />
                    <div className={`w-full md:w-3/5${darkMode ? 'bg-[#1D2226' : 'bg-white'} rounded-r-lg`}>
